...
var FTColumnflow = (function () {
	...
	function FTColumnflow(target, viewport, userConfig) {
		...
		function _findLineHeight() {
			...
			if (!config.lineHeight) {
				...
				if (!lineHeightTestContents) {
					...
					// 10 lines of text
					lineHeightTestContents = new Array(100).join('x<br />') + 'x';
				}
				...
				lineHeights = [];
				for (i = 0, l = Math.min(100, (preloadColumn.childNodes.length)); i < l; i++) {
					...
					if (!testLineHeight) {
						...
						testLineHeight = testNode.offsetHeight / 100;
		...
		function _round(val) {
			return Math.round(val * 10000) / 10000; //JD
		}
